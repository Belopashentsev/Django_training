...
class TestUserManagement(TestCase):
    ...
    def setUp(self):
        self.client = Client()
        self.superuser = ShopClient.objects.create_superuser(
            "django2", "django2@shop.local", "geekbrains"
        )
        self.user = ShopClient.objects.create_user(
            "tarantino", "tarantino@shop.local", "geekbrains"
        )
        self.user_with__first_name = ShopClient.objects.create_user(
            "uma", "uma@shop.local", "geekbrains", first_name="Ума"
        )
    ...
    def test_user_wrong_register(self):
        ...
        self.assertFormError(
            ...
            "form",
            "age",
            "Попробуйте зарегистрироваться, когда будете старше!",
        )
