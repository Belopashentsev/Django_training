...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        test_orders = (
            ...
            .order_by("action_order", "total_profit")
            .select_related("product")
        )

        for orderitem in test_orders:
            print(
                f"{orderitem.action_order:2}: заказ №{orderitem.pk:3}: {orderitem.product.name:15}: "
